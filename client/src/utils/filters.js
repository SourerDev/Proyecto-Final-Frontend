...
export function filter(properties, filter) {
  ...
  if(filter?.bathrooms?.length) {
    properties = properties.filter(p => p.bathrooms === parseInt(filter.bathrooms))
  }
  ...
  if(filter?.price?.min && filter?.price?.max) {
    properties = properties.filter(p => p.price >= filter.price.min && p.price <= filter.price.max)
  }
  if(filter?.area?.min && filter?.area?.max) {
    properties = properties.filter(p => p.area >= filter.area.min && p.area <= filter.area.max)
  }
