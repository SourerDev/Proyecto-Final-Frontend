      ...
      {payload ? (
        <div class="max-w-4xl m-1 p-2 mx-auto  grid grid-cols-1 lg:max-w-[97rem] lg:gap-x-20 lg:grid-cols-2">
          ...
          <div class="relative my-0 shadow-2xl p-3 col-start-1 row-start-1 flex flex-col-reverse rounded-lg bg-gradient-to-t from-black/75 via-black/0 sm:bg-none sm:row-start-2 sm:p-0 lg:bg-white sm:bg-white lg:row-start-1">
            ...
            <p class="text-xl mt-2 leading-4 flex justify-center font-medium text-white sm:text-slate-400 dark:sm:text-slate-400">
              {" "}
              {payload.type}
            </p>
            <p class="text-xl mt-2 leading-4 flex justify-center font-medium text-white sm:text-indigo-600 dark:sm:text-indigo-600">
              {payload.modality}
            </p>
            {isOwner && (
              <p className="flex justify-center bottom-1 left-1 bg-green-300/75 text-green-800 font-medium px-1 ">
                Eres el propietario
              </p>
            )}
          </div>
          <dl class="mt-4 text-xs font-medium drop-shadow-2xl row-start-2 sm:mt-1 sm:row-start-3 md:mt-2.5 lg:row-start-2">
            <dl className="my-4 border-black border-2 rounded-lg shadow-2xl bg-white">
              <dd class="flex justify-center  text-black  items-center">
                ...
                <span class="text-2xl ">
                  {payload.price} <span class="  font-normal">us</span>
                </span>
              </dd>
            </dl>
            <div className="shadow-2xl bg-white grid rounded-lg items-center col-start-1 col-end-3 row-start-1 sm:mb-3 sm:grid-cols-2">
              ...
              <dd class="text-black m-3 p-1 flex items-center  dark:text-black">
                ...
                <span class="m-3 text-xl p-0">
                  {payload.area} <span class="text-black text-l">.mt2</span>
                </span>
              </dd>
              <dd class="text-black m-3 p-1 flex items-center  dark:text-black">
                <svg
              ...
              <dd class="text-black m-3 p-1 flex items-center dark:text-black">
                ...
                <span class="m-3 text-xl p-0">{payload.antiquity} </span>
          ...
          <p class="mt-4 text-sm leading-6 col-start-1 sm:col-span-2 lg:mt-6 lg:row-start-4 lg:col-span-4 dark:text-slate-400">
            <article>
              <div className="px-4 my-10 shadow-2xl bg-white rounded-lg">
                <h1 class="dark:text-black underline text-2xl  m-5">
                  {" "}
                  Descripción{" "}
                </h1>
              ...
              <div className="px-4 my-10 shadow-2xl bg-white rounded-lg">
                <h1 class="dark:text-black  text-2xl underline m-5">
                  {" "}
                  Servicios{" "}
                </h1>
                {payload.services ? (
                  payload.services.map((el) => {
                    return (
                ...
                <div className="px-4">
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (
                        !user?.user_type ||
                        user.user_type === "userNotLogged"
                      ) {
                        swal
                          .fire(
                            mustBeLogged(
                              "Debes Iniciar Sesión",
                              "Para poder contactar primero debes iniciar sesión"
                            )
                          )
                          .then((result) => {
                            if (result.isConfirmed) {
                              //saveIdInLocalStorage(id,true)
                              saveInStorage("detail", {
                                id,
                                question: comment,
                              });
                              navigate("/login");
                    ...
                    className={`${
                      isOwner && "invisible"
                    } flex flex-col py-2.5 whitespace-nowrap rounded-md border border-transparent bg-indigo-600 px-4 text-base font-medium text-white shadow-sm hover:bg-indigo-700`}
                  >
                  ...
                  if (!user?.user_type || user.user_type === "userNotLogged") {
                    ...
                    swal
                      .fire(
                        mustBeLogged(
                          "Debes Iniciar Sesión",
                          "Para poder realizar una pregunta primero debes iniciar sesión"
                        )
                      )
                      .then((result) => {
                        if (result.isConfirmed) {
              ...
              >
                <div class="mb-4 w-full bg-gray-50 rounded-lg border border-gray-200 dark:bg-gray-700 dark:border-gray-600">
                  <div class="py-2 px-4 bg-white rounded-t-lg dark:bg-gray-800">
                    <label htmlFor="comment" className="text-xl m-5">
                      Preguntarle al publicador
                    </label>
                    <textarea
                      ref={refQuestion}
                  ...
                  <div class="flex justify-between items-center py-2 px-3 border-t dark:border-gray-600">
                    ...
                    <p class="ml-auto text-xs text-gray-500 dark:text-gray-400">
                      Cumpla con nuestras políticas sobre el uso indebido del
                      vocabulario.
                      <a
                        href="#"
