...
const ContentDashboard = () => {
  ...
  useEffect(() => {
    callsApi.getProperties().then((res) => {
      ...
      if (user.user_type === 'userPremiun') {
        let filterData = payload.filter((p) => p.User.email === user.email)
  ...
  return (
    <>
      <div className="flex min-h-full w-full justify-center p-1 ">
        <div className="w-full">
          <table className="w-full border-separate space-y-6 text-sm text-white ">
            <thead className="text-white-500 bg-gray-800">
            ...
            <tbody>
              {!propertiesRender.length ? (
                ...
                propertiesRender.map((el) => (
                  <Dashboard
                    key={el.id}
                    data={el}
                    owner={user.user_type === 'admin' ? true : false}
                    deleted={handleDeleteProperty}
                    disabled={handleDisabledProperty}
                    reset={() => {}}
                  />
                ))
              )}
