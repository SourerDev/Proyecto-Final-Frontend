...
export function PropertiesCarousel({ _type = 'sale' }) {
  ...
  return (
    <div className='px-4 mb-4 '>
      ...
      <div className='flex'>
        {TABS.map( tab => <button className={`px-6 py-1 mt-1 rounded-t bg-gray-800 text-left text-lg font-medium  text-white mb-2 border-b-2 ${tab === type ? ' border-violet-400': 'bg-gray-800/70 border-transparent'} `} onClick={() => setType(tab)} key={tab}>{tab}</button>)}
      </div>
      <Carousel>
        {properties.map(({ Property,  ...property }, i) => (
          <Item className="px-0.5 overflow-hidden" key={i} onClick={() => navigate(`/properties/${property.idPublication}`)}>
